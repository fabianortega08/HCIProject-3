...
def main():
    ...
    with tasks:
        ...
        selected_task = st.selectbox("Select Task",
                                     ["Navigate to the Overview tab and watch a match video",
                                    "Use the Players tab to select three teams of your choosing and view their team players ",
                                    "Interpret data  in the Standings tab"])


        st.write("Task Description:  Complete the selected task using the timer bellow.")
        ...
        success = st.radio("Was the task completed successfully?", ["Yes", "No"])
        notes = st.text_area("Observer Notes", placeholder="Write your feedback here")
        participant_notes = st.text_area("Participant Notes",
                                         placeholder="How was your experience performing this task?")
